...
import type { PNDData, YearData } from '../types/pnd-types'; // Your types
...
export const BrushChart = ({ pndData, allLabels, onRangeChange, initialMinLabel, initialMaxLabel }: BrushChartProps) => {
    ...
    const brushChartData: ChartData<'line'> = useMemo(() => {
        const { currentYearData, currentYear } = pndData;
        const createDataMap = (data: YearData[]): Map<string, number> =>
            new Map(data.map(item => [item.Fecha.substring(5), item.average_PML]));
